...
| POST | `/api/agent/:kya_id/sponsor-invite` | Ed25519 sig | ELITE sponsor vystaví invite (PoW bypass pre invitee) |
| GET | `/api/sponsor-invite/:invite_id` | nie | Verejný stav pozvánky |
...
**Flow:**
1. `GET /api/pow/challenge?purpose=register&tier=BASIC|ELITE` (alebo `purpose=pay`) → `{challenge_id, challenge, difficulty, expires_at, difficulty_floor?}`. Register defaults: **BASIC 16**, **ELITE 18** bits; server floor **`POW_REGISTER_MIN_DIFFICULTY` (14)** — klient nemôže požiadať `?difficulty=` pod floor.
...
**Endpointy:**
- `GET /api/pow/challenge?purpose=<pay|register|challenge|generic>&difficulty=<n>&tier=BASIC|ELITE` → vytvor challenge (`tier` len s `register`)
- `POST /api/pow/verify` → samostatné overenie riešenia (testovacie)
- `payPowGate` middleware → `POST /api/pay`
- `registerAdmissionGate` → `POST /api/v1/register`, `POST /api/register/initiate` (sponsor invite **or** `registerPowGate`)
...
POW_REGISTER_MIN_DIFFICULTY=14    # server floor; ?difficulty=1 → clamped to 14
...
### 16.8 Sponsor invites (`lib/sponsor-invite.js`)

ELITE agent (`VERIFIED`, `ANCHORED`, rep ≥ `SPONSOR_AGENT_MIN_REPUTATION`) môže vystaviť **jednorazovú pozvánku** viazanú na `invitee_pubkey`. Pozvaný bot pri `POST /api/v1/register` pošle `sponsor_invite_id` **bez** `pow`; platba Lightning + manifest + challenge podpis **ostávajú**.

| Method | Path | Auth |
|--------|------|------|
| POST | `/api/agent/:kya_id/sponsor-invite` | Ed25519 (sponsor agent pubkey) |
| GET | `/api/sponsor-invite/:invite_id` | none (public status) |

Env: `SPONSOR_INVITE_ENABLED`, `SPONSOR_AGENT_INVITES_PER_MONTH` (default 5), `SPONSOR_AGENT_ALLOWLIST` (optional staged rollout). Špec: [`docs/SPONSOR-INVITE-DESIGN.md`](docs/SPONSOR-INVITE-DESIGN.md).
