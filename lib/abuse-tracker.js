...
// Escalation: repeated protocol violations within a sliding time window get harsher.
// Designed to be audit-friendly (each step is still a PROTOCOL_VIOLATION event).
const PROTOCOL_ESCALATION = {
    windowHours: parseInt(process.env.PROTOCOL_VIOLATION_WINDOW_HOURS || '24', 10),
    // 1st within window uses caller-provided delta (e.g. -50 / -100).
    // 2nd within window becomes -200.
    // 3rd+ within window becomes -500 (usually forces SUSPENDED for BASIC-tier agents).
    secondDelta: parseInt(process.env.PROTOCOL_VIOLATION_SECOND_DELTA || '-200', 10),
    thirdPlusDelta: parseInt(process.env.PROTOCOL_VIOLATION_THIRD_PLUS_DELTA || '-500', 10),
};

async function _escalateProtocolDelta(client, { kya_id, baseDelta, evidence }) {
    const hours = Math.max(1, Math.min(PROTOCOL_ESCALATION.windowHours || 24, 168)); // cap 7 days
    const r = await client.query(
        `SELECT COUNT(*)::int AS n
         FROM reputation_events
         WHERE kya_id = $1
           AND event_type = 'PROTOCOL_VIOLATION'
           AND occurred_at > NOW() - INTERVAL '${hours} hours'`,
        [kya_id]
    );
    const n = r.rows?.[0]?.n || 0;
    const withinWindowCount = Number(n);
    // n is the count BEFORE we add this new event.
    if (withinWindowCount >= 2) {
        return { delta: PROTOCOL_ESCALATION.thirdPlusDelta, tier: 'third_plus', prior_count: withinWindowCount, evidence };
    }
    if (withinWindowCount === 1) {
        return { delta: PROTOCOL_ESCALATION.secondDelta, tier: 'second', prior_count: withinWindowCount, evidence };
    }
    return { delta: baseDelta, tier: 'first', prior_count: withinWindowCount, evidence };
}
    ...
    try {
        ...
        if (n >= CFG.BAD_SIG_PER_HOUR_THRESHOLD) {
            ...
            try {
                ...
                const esc = await _escalateProtocolDelta(client, {
                    kya_id,
                    baseDelta: CFG.BAD_SIG_AUTO_SLASH_DELTA,
                ...
                await repEngine.applyEvent(client, {
                    ...
                    source: 'system',
                    delta: esc.delta,
                    reason: `Auto-slash: ${n} signature failures za hodinu (threshold ${CFG.BAD_SIG_PER_HOUR_THRESHOLD}); escalation=${esc.tier} prior_protocol_violations=${esc.prior_count}`,
                    evidence: { ...esc.evidence, escalation: esc.tier, prior_protocol_violations: esc.prior_count },
                });
        ...
        for (const row of r.rows) {
            try {
                ...
                try {
                    ...
                    const esc = await _escalateProtocolDelta(client, {
                        kya_id: row.kya_id,
                        baseDelta: CFG.ANOMALY_AUTO_SLASH_DELTA,
                    ...
                    await repEngine.applyEvent(client, {
                        ...
                        source: 'system',
                        delta: esc.delta,
                        reason: `Anomaly: ${row.n}× same target "${String(row.target).slice(0, 40)}" za hodinu; escalation=${esc.tier} prior_protocol_violations=${esc.prior_count}`,
                        evidence: { ...esc.evidence, escalation: esc.tier, prior_protocol_violations: esc.prior_count },
                    });
                    await client.query('COMMIT');
                    stats.slashed++;
