...
async function connect(logger = console, options = {}) {
    const uri = (options.nwcUri || NWC_URI || '').trim();
...
async function payInvoice({ invoice, amountSats, metadata }) {
    ...
    const result = await nwc.payInvoice(req);
    return {
