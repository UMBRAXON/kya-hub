    ...
    if (agent.last_heartbeat_at) {
        ...
        if (lastHbAgo < DAY_MS && lastScoreChangeAgo > 7 * DAY_MS) {
            ...
            try {
                ...
                const lbResult = await repEngine.applyEvent(client, {
                    agent_id: agent.id,
                    kya_id: agent.kya_id,
                ...
                if (lbResult && (lbResult.delta !== 0 || lbResult.oldZone !== lbResult.newZone)) {
                    developerWebhooks.emit(pool, {
                        event: 'reputation.changed',
                        kya_id: agent.kya_id,
                        payload: {
                            event_id: lbResult.eventId,
                            delta: lbResult.delta,
                            old_score: lbResult.oldScore,
                            new_score: lbResult.newScore,
                            old_zone: lbResult.oldZone,
                            new_zone: lbResult.newZone,
                            source: 'decay_loyalty',
                        },
                    }).catch(() => {});
                }
                stats.loyalty_bonus++;
        ...
        if (result) {
            ...
            if (revoked) {
                developerWebhooks.emit(pool, {
                    event: 'cert.revoked',
                    kya_id: agent.kya_id,
                    payload: { side_effects: result.sideEffects, source: 'decay', event_type: eventType },
                }).catch(() => {});
                stats.suspended++;
