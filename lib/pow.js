...
//
// Difficulty:
//   - pay/default 18 bits → ~250k pokusov v priemere → ~1-2 s na CPU
//   - register default 16 (POW_REGISTER_DIFFICULTY) — lighter for weak bots
//   - register floor 14 (POW_REGISTER_MIN_DIFFICULTY) — client ?difficulty= cannot go below
//   - register ELITE default 18 (POW_REGISTER_ELITE_DIFFICULTY) when tier=ELITE
//   - admin môže zvýšiť obtiažnosť pod útokom; register min zostáva v platnosti
//
// PoW gate: POW_REQUIRED_FOR (default pay,register). Heartbeat/actions never use PoW.
// ============================================================================
...
/** BASIC / unspecified tier register default (lighter than pay). */
const REGISTER_DIFFICULTY = _parseDifficulty(process.env.POW_REGISTER_DIFFICULTY, 16);
/** ELITE register default (tier=ELITE on challenge). */
const REGISTER_ELITE_DIFFICULTY = _parseDifficulty(
    process.env.POW_REGISTER_ELITE_DIFFICULTY,
    DEFAULT_DIFFICULTY
);
/** Server floor for purpose=register (?difficulty= cannot go below). */
const REGISTER_MIN_DIFFICULTY = _parseDifficulty(process.env.POW_REGISTER_MIN_DIFFICULTY, 14);
...
const CFG = {
    DEFAULT_DIFFICULTY,
    REGISTER_DIFFICULTY,
    REGISTER_ELITE_DIFFICULTY,
    REGISTER_MIN_DIFFICULTY,
    MAX_DIFFICULTY,
...
/**
 * Resolve leading-zero-bit difficulty for a challenge (server-side clamp).
 * @param {object} opts
 * @param {string} opts.purpose
 * @param {number|undefined} opts.requested - client ?difficulty= or undefined for tier default
 * @param {string|undefined} opts.tier - BASIC | ELITE (register only)
 * @returns {{ difficulty: number, difficulty_requested: number|null, difficulty_floor: number|null, tier: string|null }}
 */
function resolveDifficulty({ purpose = 'generic', requested, tier } = {}) {
    let baseDefault = CFG.DEFAULT_DIFFICULTY;
    let minFloor = 1;
    let tierNorm = null;

    if (purpose === 'register') {
        minFloor = CFG.REGISTER_MIN_DIFFICULTY;
        tierNorm = tier === 'ELITE' ? 'ELITE' : (tier === 'BASIC' ? 'BASIC' : null);
        baseDefault = tierNorm === 'ELITE'
            ? CFG.REGISTER_ELITE_DIFFICULTY
            : CFG.REGISTER_DIFFICULTY;
        // Ensure configured defaults respect floor
        baseDefault = Math.max(baseDefault, minFloor);
    } else if (purpose === 'pay') {
        baseDefault = CFG.DEFAULT_DIFFICULTY;
    }

    let requestedNum = null;
    if (requested !== undefined && requested !== null && requested !== '') {
        const parsed = typeof requested === 'number' ? requested : parseInt(String(requested), 10);
        if (Number.isFinite(parsed)) requestedNum = parsed;
    }

    const raw = requestedNum !== null ? requestedNum : baseDefault;
    const difficulty = Math.min(Math.max(raw, minFloor), CFG.MAX_DIFFICULTY);

    return {
        difficulty,
        difficulty_requested: requestedNum,
        difficulty_floor: purpose === 'register' ? minFloor : null,
        tier: tierNorm,
    };
}
 ...
 */
async function createChallenge(pool, { purpose = 'generic', difficulty, clientIp, tier } = {}) {
    if (!PURPOSES.includes(purpose)) purpose = 'generic';
    const tierOpt = tier === 'ELITE' || tier === 'BASIC' ? tier : undefined;
    const resolved = resolveDifficulty({
        purpose,
        requested: difficulty,
        tier: purpose === 'register' ? tierOpt : undefined,
    });
    const diff = resolved.difficulty;
    ...
    return {
        ...
        purpose,
        tier: resolved.tier,
        difficulty_requested: resolved.difficulty_requested,
        difficulty_floor: resolved.difficulty_floor,
