...
function register(app, deps) {
    ...
    app.get('/api/protocol/integrator-sandbox', (req, res) => {
        res.json({
            ...
            no_database: true,
            production_note:
                'On production hub, UMBRA-TEST-* returns 400 on /api/v1/agents/* — use this doc endpoint only.',
            fixtures: {
            ...
            endpoints: {
                status: 'GET /api/v1/agents/UMBRA-TEST-0001/status (non-production only)',
            ...
            const agents = await pool.query(
                `SELECT COUNT(*)::int AS production_agents
                 ...
                 WHERE a.payment_settled_at IS NOT NULL
                   AND a.agent_name !~* '^(TEST-|GOV-|DEMO-|REPTEST|ABTEST)'
                   AND a.agent_name !~* 'UMBRAXONTEST|TEST-WEBHOOK|TEST-BOT'
                   AND a.kya_id !~* '^UMBRA-TEST-'`,
            );
