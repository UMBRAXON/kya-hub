...
def self_test() -> int:
    ...
    # 3) PoW solver — solve a low-difficulty challenge and check the digest manually
    challenge = "00" * 32
    ...
    # 4) Ed25519 sign/verify roundtrip (only if PyNaCl present)
    if HAVE_NACL:
        seed = bytes(range(32))
...
def _build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        description="UMBRAXON KYA-Hub Python referenčný klient (registrácia + action + PoW solver).",
    )
